<script lang="ts" setup>
  ...
  const seconds: Ref<number | null> = ref(null);
  const progress = computed(() => {
    if (seconds.value === null) {
      return null;
    }
    return (
      (100 * seconds.value) / (props.currentPerformance.endSec - props.currentPerformance.startSec)
    );
  });
        ...
        >
          <v-row class="text-center justify-center">
            <v-col class="ma-auto" cols="1">
              <v-btn icon="mdi-share-variant" variant="text" @click="shareYoutubeRef?.popup()" />
            </v-col>
            <v-col cols="10">
              <v-list-item-title>
          ...
          <v-row>
            ...
            <v-col class="pa-0 text-caption" cols="auto">{{ secondsFromStart }}</v-col>
            <v-col />
            <v-col class="pa-0 text-caption" cols="auto">{{ secondsToEnd }}</v-col>
          </v-row>
          <v-row>
            <v-col cols="auto">
            ...
            <v-col class="text-center">
              <v-btn icon="mdi-rewind" variant="text" @click="onRewindClickedEvent" />
              <template v-if="playing">
                <v-btn icon="mdi-pause" variant="text" @click="onPause" />
              </template>
              <template v-else>
                <v-btn icon="mdi-play" variant="text" @click="onPlay" />
              </template>
              <v-btn icon="mdi-fast-forward" variant="text" @click="emits('next')" />
            </v-col>
            <v-col cols="auto">
